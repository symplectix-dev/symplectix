...
async fn command_variant_and_its_command_resolve_from_store() {
    ...
    let mut command = ply::Command::new("python3");
    command.arg("main.py");
    let command_digest = store.put(&command).await.unwrap();

    let function = ply::Function::command(command_digest);
    ...
    let resolved_function: ply::Function = store.get(&function_digest).await.unwrap().unwrap();
...
async fn map_variant_and_its_command_and_config_resolve_from_store() {
    ...
    let mut command = ply::Command::new("serve");
    command.arg("--stdio");
    let command_digest = store.put(&command).await.unwrap();
    ...
    let function = ply::Function::map(command_digest, config_digest);
    let function_digest = store.put(&function).await.unwrap();
    ...
    let (resolved_command_digest, resolved_config_digest) = match resolved_function {
        ply::Function::Map { command, config } => (command, config),
