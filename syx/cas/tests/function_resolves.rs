...
//! `cas::Storage`, using only its own digest.
...
async fn action_variant_and_its_command_and_config_resolve_from_store() {
    ...
    // the store.
    let file_digest = store.put(&Bytes::from_static(b"threshold: 10")).await.unwrap();
    ...
    assert_eq!(store.get(&file_digest).await.unwrap(), Some(Bytes::from_static(b"threshold: 10")));
}
...
async fn server_variant_and_its_command_and_config_resolve_from_store() {
    ...
    // The config: a Tree with one file entry, itself resolvable from
    // the store.
    let file_digest = store.put(&Bytes::from_static(b"port: 8080")).await.unwrap();
    let config = Tree::new([("config.yaml".to_string(), Node::Blob(file_digest))], []);
    let config_digest = store.put(&config).await.unwrap();
    ...
    // The file the config tree references is itself resolvable.
    assert_eq!(store.get(&file_digest).await.unwrap(), Some(Bytes::from_static(b"port: 8080")));
}
