...
#
# Usage: fc_start.sh <ssh_key_file>
set -euo pipefail
...
fc_api -X PUT http://localhost/boot-source -d \
  "{\"kernel_image_path\": \"$FC_DATA_DIR/vmlinux\", \"initrd_path\": \"$FC_DATA_DIR/initrd.cpio.gz\", \"boot_args\": \"console=ttyS0 reboot=k panic=1\"}"

fc_api -X PUT http://localhost/drives/rootfs -d \
  "{\"drive_id\": \"rootfs\", \"path_on_host\": \"$FC_DATA_DIR/rootfs.squashfs\", \"is_root_device\": false, \"is_read_only\": true}"
...
fc_api -X PUT http://localhost/mmds -d \
  "{\"fc_ipv4\": \"$FC_IPV4\", \"prefix4\": $PREFIX4, \"fc_ipv6\": \"$FC_IPV6\", \"prefix6\": $PREFIX6, \"tap_ipv4\": \"$TAP_IPV4\", \"tap_ipv6\": \"$TAP_IPV6\", \"ssh_public_key\": \"$SSH_KEY\"}"
