//! Content stored in a `Forgetter` is preserved faithfully.
...
async fn content_put_is_returned_unchanged_by_get() {
    ...
    assert_eq!(forgetter.cas().get(&d).await.unwrap(), Some(cas::Bytes::from_static(b"hello")));
}
...
async fn copy_from_accepts_a_file_and_streams_it_in() {
    ...
    let d = forgetter.cas().copy_from(len, &mut file).await.unwrap();
