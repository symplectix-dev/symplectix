//! A `Forgetter`'s returned digest doesn't depend on which method
...
async fn copy_from_file_produces_the_same_digest_as_put() {
    ...
    let from_reader = forgetter.cas().copy_from(len, &mut file).await.unwrap();
    let from_bytes = forgetter.cas().put(&cas::Bytes::from_static(b"hello")).await.unwrap();
...
async fn copy_from_bytes_produces_the_same_digest_as_put() {
    ...
    let d = forgetter.cas().copy_from(content.len() as u64, &mut cursor).await.unwrap();
    assert_eq!(d, cas_testing::digest_bytes(b"hello"));
    assert_eq!(forgetter.cas().get(&d).await.unwrap(), Some(content));
}
