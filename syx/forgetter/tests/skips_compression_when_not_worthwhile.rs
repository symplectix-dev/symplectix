//! A `Forgetter` round-trips large content correctly regardless of
//! whether it ends up compressed. (Whether compression was actually
//! applied is an internal decision, covered by unit tests inside
//! `content_addressing`'s own `Codec`.)
...
async fn large_incompressible_content_streamed_via_copy_from_round_trips() {
    ...
    assert_eq!(forgetter.cas().get(&d).await.unwrap(), Some(cas::Bytes::from(content)));
}
