...
/// a fresh `TempDir`.
async fn open(dir: impl AsRef<Path>) -> Logger {
    let (logger, mut replayed) =
        Logger::open(dir.as_ref(), u16::MAX, u64::MAX, None).await.unwrap();
...
async fn save_then_find_returns_the_same_bytes() {
    ...
    let logger = open(dir.path()).await;
    let value = Bytes::from_static(b"hello");

    let locator = logger.save(value.clone()).await.unwrap();
...
async fn find_returns_none_for_an_unknown_file() {
    ...
    assert!(logger.find(file_id::next()).await.is_none());
}
...
async fn active_segment_len_tracks_bytes_written_to_the_active_segment() {
    ...
    assert_eq!(logger.active_segment_len(), 0);
    logger.save(value).await.unwrap();
    assert_eq!(logger.active_segment_len(), expected);
}
...
async fn rotate_moves_the_active_segment_into_pending_and_resets_active_segment_len() {
    ...
    let segment = logger.rotate().await.unwrap();

    assert_eq!(logger.active_segment_len(), 0);
    assert_eq!(logger.pending_segments(), vec![segment]);
    assert!(matches!(logger.find(segment).await, Some(Found::Sealed(_))));
    assert_eq!(read(&locator).await, value);
}
...
async fn rotate_notifies_rotated() {
    let dir = testing::tempdir();
    let logger = open(dir.path()).await;
    let mut rotated = logger.rotated();
    logger.save(Bytes::from_static(b"hello")).await.unwrap();
...
#[tokio::test]
async fn dropping_logger_closes_rotated() {
    let dir = testing::tempdir();
    let logger = open(dir.path()).await;
    let mut rotated = logger.rotated();
    ...
    tokio::time::timeout(std::time::Duration::from_millis(200), rotated.changed())
        .await
        .expect("changed should resolve promptly")
        .expect_err("rotated should report closed once Logger is dropped");
}
...
async fn save_rotates_the_active_segment_on_its_own_once_rotate_threshold_is_crossed() {
    ...
    let (logger, _) = Logger::open(dir.path(), u16::MAX, threshold, None).await.unwrap();
    let mut rotated = logger.rotated();

    let locator = logger.save(value.clone()).await.unwrap();

    assert_eq!(logger.active_segment_len(), 0);
    assert_eq!(logger.pending_segments(), vec![locator.file()]);
    assert!(matches!(logger.find(locator.file()).await, Some(Found::Sealed(_))));
    assert_eq!(read(&locator).await, value);
    tokio::time::timeout(std::time::Duration::from_millis(200), rotated.changed())
        .await
        .expect("changed should resolve promptly")
...
async fn save_rotates_the_active_segment_on_its_own_once_rotate_after_elapses() {
    ...
    let value = Bytes::from_static(b"hello");
    let (logger, _) =
        Logger::open(dir.path(), u16::MAX, u64::MAX, Some(std::time::Duration::from_millis(20)))
            .await
            .unwrap();
    let locator = logger.save(value.clone()).await.unwrap();
    assert!(matches!(logger.find(locator.file()).await, Some(Found::Active(_))));
...
async fn find_reports_active_vs_pending_correctly() {
    let dir = testing::tempdir();
    let logger = open(dir.path()).await;
    let locator = logger.save(Bytes::from_static(b"hello")).await.unwrap();
...
async fn forget_deletes_the_segment() {
    ...
    logger.forget(segment).await.unwrap();

    assert!(logger.pending_segments().is_empty());
    assert!(logger.find(segment).await.is_none());
}
...
async fn reopening_finds_pending_segments_left_by_a_previous_instance() {
    ...
    let (segment, locator) = {
        let logger = open(dir.path()).await;
        let locator = logger.save(value.clone()).await.unwrap();
        (logger.rotate().await.unwrap(), locator)
    };

    let (reopened, mut replayed) =
        Logger::open(dir.path(), u16::MAX, u64::MAX, None).await.unwrap();
...
async fn reopening_drops_a_torn_tail_record_and_keeps_the_valid_ones() {
    ...
    {
        let logger = open(dir.path()).await;
        logger.save(value.clone()).await.unwrap();
    }
...
async fn reopening_leaves_a_short_foreign_file_untouched() {
    ...
    // can't possibly hold `MAGIC`. Guessing this is one of `logger`'s
...
async fn reopening_leaves_an_empty_foreign_file_untouched() {
    ...
    // 0 bytes is not proof this is one of `logger`'s own.
...
async fn save_refuses_once_max_pending_segments_are_stuck() {
    let dir = testing::tempdir();
    let (logger, _) = Logger::open(dir.path(), 2, u64::MAX, None).await.unwrap();
    ...
    for payload in [&b"a"[..], &b"b"[..]] {
        logger.save(Bytes::copy_from_slice(payload)).await.unwrap();
        logger.rotate().await.unwrap();
    }
    assert_eq!(logger.pending_segments().len(), 2);

    let value = Bytes::from_static(b"c");
    let err = logger.save(value.clone()).await.unwrap_err();
    assert_eq!(err.kind(), io::ErrorKind::Other);

    // Forgetting one segment frees a slot for the next write.
    let oldest = logger.pending_segments()[0];
    logger.forget(oldest).await.unwrap();
    let locator = logger.save(value.clone()).await.unwrap();
    assert_eq!(read(&locator).await, value);
}
